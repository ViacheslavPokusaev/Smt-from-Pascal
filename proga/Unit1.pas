...
WordApplication1.Selection.InsertAfter('Название теста'+' '+'-'+' '+Form1.Caption + #13);
WordApplication1.Selection.InsertAfter('ФИО'+' '+'-'+' '+n + #13);
WordApplication1.Selection.InsertAfter('Группа'+' '+'-'+' '+n2 + #13);
WordApplication1.Selection.InsertAfter('Дата и время начала теста'+' '+'-'+' '+c + #13);
WordApplication1.Selection.InsertAfter('Общее количество заданных вопросов'+' '+'-'+' '+inttostr(g2)+ #13);
WordApplication1.Selection.InsertAfter('Процент правильных ответов'+' '+'-'+' '+floattostr((u/g2)*100)+'%' + #13);
...
begin
if t=True then begin Action:=canone;ShowMessage('Пройдите тестирование');
end;end;
...
c:=datetimetostr(Now);
n:=inputbox('Введите ФИО',' ',' ');
n2:=inputbox('Введите группу',' ',' ');
end;
...
timer1.Interval:=0;
g3:=0;
statusbar1.Panels[0].Text:='Тест закончился';
end;
if g3=0 then g3:=i;
